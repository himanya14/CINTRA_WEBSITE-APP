...
function cleanGraphData(
  ...
  nodes.forEach(
    (node) => {
      ...
      /*
       * Keep one factual FIR/case context node. Mobile evidence belongs to an
       * existing case, so showing that hub makes the whole person/entity/
       * evidence network readable instead of leaving a lone evidence node.
       * All case nodes collapse to one visual node; this does not imply guilt.
       */
      if (isCaseNode) {
        key = "case:context";
        nextNode = {
